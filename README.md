...
### 🎯 未获取基质统计（v0.9.1 新增）
- **全局排名**：按刷取组合（3 能力值 + 1 属性/系列技能）统计缺口，取前 36 名
...
**表格结构**：14 系列技能 × 5 能力值 = 70 列，分两段渲染（强攻~残暴 / 附术~效益）。

**刷取机制**：每个地区提供 8 属性 + 8 系列技能，每次刷取选择 3 能力值 + 1 属性或 1 系列技能，从 24 种候选中随机获得 3 个。
...
每个刷取组合对应 24 格（3 能力值 × 8 属性/系列技能）。每格的"缺口贡献"：
