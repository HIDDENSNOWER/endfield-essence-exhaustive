...
> 适用版本：Ver.0.7.1 ("再访基地"更新) ｜ 最后更新：与 v0.7.1 代码同步
...
### 3.2 核心依赖关系（谁依赖谁）
    ...
    storage.js     ← datasetManager / datasetRemark / importExport / datasetMerge / defaultLoader /
                      ...
                      schemeManager / stateColorSchemeManager / history
...
### 4.2 统一事件绑定（events.js `bindAllEvents()` 调用 14+ 个模块）
...
> 例外：`note.js` 在 `initNoteFeature()` 内自绑事件；`interfaceColors.js` 在 `renderPanel()` 时动态绑定控件事件（颜色输入框内联 `oninput`/`onchange`）。
...
### 6.10 主题与个性化
    ...
    单元格状态颜色：HEX/RGB/RGBA/CMYK/HSLA 编辑；分量自动钳制；Alpha 通道暂不支持
    界面颜色：所有颜色变量分组管理；CSS 变量驱动预览，修改实时反映；支持完整主界面复刻预览
    方案管理：
      · 界面颜色方案与单元格颜色方案独立
      · 内置白天/黑夜默认方案（系统方案不可修改）
      · 用户修改并应用后自动保存为"用户自定义方案"
      · 支持另存、导入（格式检测+对比预览+重定向）、导出（自定义文件名）
...
- **`interfaceColors.js` 颜色输入框使用内联事件**（`oninput`/`onchange`），动态渲染控件后直接绑定，无需额外事件委托
